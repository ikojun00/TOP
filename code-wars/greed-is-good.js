...
This is something that you should never do. If 
you mutate the input, you will not be able to pass all the tests.
*/

function score( dice ) {
    let result = 0;
    let uniqueElements = [...new Set(dice)];
    const elementCounts = uniqueElements.map(value => [value, dice.filter(str => str === value).length]);
    console.log(dice)
